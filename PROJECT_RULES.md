...
- 询问用户是否将方案保存到 `docs/plans/` 目录下。
- 若当前处于plan mode，则不要执行任何覆写操作，如使用bash，而是让用户切换到build mode后再进行文件保存。
...
- 每次内容更新后，不要自动执行Git提交或推送，而是询问用户，若用户允许才执行操作。
- commit的Message应该使用中文来描述具体更改内容，比如：feat: 完成XX功能开发。
